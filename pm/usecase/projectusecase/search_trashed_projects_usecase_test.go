...
func TestSearchProjectsUsecaseSearchTrashedProjects(t *testing.T) {
	type fields struct {
	...
	tests := []struct {
		...
		args        args
		want        *projectoutput.SearchTrashedProjectsOutput
		wantErr     error
	}{
		{
			name: "正常",
			prepareMock: func(f *fields) error {
				...
				projectsDto := []*projectoutput.SearchTrashedProjectOutput{
					{
				...
				f.projectQueryService.EXPECT().CountTrashedProjectsByKeyNameAndName(ctx, productIDVo, keyword).Return(totalCount, nil)
				f.projectQueryService.EXPECT().SearchTrashedProjects(ctx, productIDVo, keyword, limit, offset).Return(projectsDto, nil)
		...
		{
			name: "DBエラー(CountTrashedProjectsByKeyNameAndName)",
			prepareMock: func(f *fields) error {
				ctx := context.TODO()
				...
				f.productRepository.EXPECT().FetchProductByIDForUpdate(ctx, productIDVo).Return(nil, err)
				f.projectQueryService.EXPECT().CountTrashedProjectsByKeyNameAndName(ctx, productIDVo, keyword).Return(uint32(0), apperr)
		...
		{
			name: "DBエラー(SearchTrashedProjects)",
			prepareMock: func(f *fields) error {
				ctx := context.TODO()
				...
				f.productRepository.EXPECT().FetchProductByIDForUpdate(ctx, productIDVo).Return(nil, err)
				f.projectQueryService.EXPECT().CountTrashedProjectsByKeyNameAndName(ctx, productIDVo, keyword).Return(totalCount, nil)
				f.projectQueryService.EXPECT().SearchTrashedProjects(ctx, productIDVo, keyword, limit, offset).Return(nil, apperr)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			u := NewSearchTrashedProjectsUsecase(f.projectQueryService, f.productRepository)

			got, err := u.SearchTrashedProjects(tt.args.ctx, tt.args.in)
			if hasErr, expectErr := err != nil, tt.wantErr != nil; hasErr != expectErr {
				t.Errorf("SearchTrashedProjects() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
