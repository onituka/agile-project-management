...
func TestUpdateProjectNoteUsecaseUpdateProjectNote(t *testing.T) {
	type fields struct {
	...
	}{
		{
			...
			prepareMock: func(f *fields) error {
				...
				gomock.InOrder(
					...
					f.projectNoteRepository.EXPECT().FetchProjectNoteByID(ctx, projectNoteIDVo, projectIDVo).Return(oldProjectNoteDm, err),
				)

				f.projectNoteRepository.EXPECT().FetchProjectNoteByProjectIDAndTitle(ctx, projectIDVo, titleVo).Return(nil, apperr)
				f.projectNoteRepository.EXPECT().UpdateProjectNote(ctx, gomock.Any()).Return(nil)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			u := NewUpdateProjectNoteUsecase(f.projectNoteRepository, f.productRepository, f.projectRepository)

			got, err := u.UpdateProjectNote(tt.args.ctx, tt.args.in)
			if hasErr, expectErr := err != nil, tt.wantErr != nil; hasErr != expectErr {
				t.Errorf("UpdateProjectNote() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
