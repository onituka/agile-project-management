...
func Test_fetchProjectNotesUsecase_FetchProjectNotes(t *testing.T) {
	type fields struct {
	...
	tests := []struct {
		...
		args        args
		want        *projectnoteoutput.FetchProjectNotesOutput
		wantErr     error
	}{
		{
			name: "正常",
			prepareMock: func(f *fields) error {
				...
				projectNotesDto := []*projectnoteoutput.ProjectNoteOutput{
					{
				...
				f.projectNoteQueryService.EXPECT().CountProjectNotesByProductIDAndProjectID(ctx, productIDVo, projectIDVo).Return(totalCount, nil)
				f.projectNoteQueryService.EXPECT().FetchProjectNotes(ctx, productIDVo, projectIDVo, limit, offset).Return(projectNotesDto, nil)
		...
		{
			name: "DBエラー(CountProjectNotesByProductIDAndProjectID)",
			prepareMock: func(f *fields) error {
				ctx := context.TODO()
				var err error
				...
				f.productRepository.EXPECT().FetchProductByIDForUpdate(ctx, productIDVo).Return(nil, err)
				f.projectRepository.EXPECT().FetchProjectByIDForUpdate(ctx, projectIDVo, productIDVo).Return(nil, err)
				f.projectNoteQueryService.EXPECT().CountProjectNotesByProductIDAndProjectID(ctx, productIDVo, projectIDVo).Return(uint32(0), apperr)
		...
		},
		{
			...
			prepareMock: func(f *fields) error {
				...
				f.productRepository.EXPECT().FetchProductByIDForUpdate(ctx, productIDVo).Return(nil, err)
				f.projectRepository.EXPECT().FetchProjectByIDForUpdate(ctx, projectIDVo, productIDVo).Return(nil, err)
				f.projectNoteQueryService.EXPECT().CountProjectNotesByProductIDAndProjectID(ctx, productIDVo, projectIDVo).Return(totalCount, err)
				f.projectNoteQueryService.EXPECT().FetchProjectNotes(ctx, productIDVo, projectIDVo, limit, offset).Return(nil, apperr)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			u := NewFetchProjectNotesUsecase(f.projectNoteQueryService, f.productRepository, f.projectRepository)

			got, err := u.FetchProjectNotes(tt.args.ctx, tt.args.in)
			if hasErr, expectErr := err != nil, tt.wantErr != nil; hasErr != expectErr {
				t.Errorf("FetchprojectNotes() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
