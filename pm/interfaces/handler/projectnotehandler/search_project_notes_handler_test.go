...
func TestSearchProjectNotesHandlerSearchProjectNotes(t *testing.T) {
	type fields struct {
		searchProjectNotesUsecase *mockprojectnoteusecase.MockSearchProjectNotesUsecase
	}
	tests := []struct {
		name           string
		fileSuffix     string
	...
	}{
		{
			...
			prepareMock: func(f *fields) {
				...
				out := &projectnoteoutput.SearchProjectNotesOutput{
					TotalCount: 2,
				...
				f.searchProjectNotesUsecase.EXPECT().SearchProjectNotes(ctx, in).Return(out, nil)
			},
			prepareRequest: func(r *http.Request) {
		...
		},
		{
			...
			prepareMock: func(f *fields) {
				...
				f.searchProjectNotesUsecase.EXPECT().SearchProjectNotes(ctx, in).Return(nil, err)
			},
			prepareRequest: func(r *http.Request) {
				*r = *mux.SetURLVars(r, map[string]string{
					"productID": "4495c574-34c2-4fb3-xca4-3a7c79c267a6",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			f := fields{
				searchProjectNotesUsecase: mockprojectnoteusecase.NewMockSearchProjectNotesUsecase(gmctrl),
			}
			...
			h := NewSearchProjectNotesHandler(f.searchProjectNotesUsecase)

			r := httptest.NewRequest(http.MethodGet, "/products/{productID:[a-z0-9-]{36}}/projects/{projectID:[a-z0-9-]{36}}/notes/search", nil)
