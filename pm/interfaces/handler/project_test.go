	...
	tests := []struct {
	...
	}{
		{
			...
			prepareMock: func(f *fields) {
				...
				f.projectUsecase.EXPECT().UpdateProject(ctx, in).Return(out, nil)
			},
			prepareRequest: func(r *http.Request) {
				*r = *mux.SetURLVars(r, map[string]string{
					"projectID": "024d71d6-1d03-11ec-a478-0242ac180002",
		...
		},
		{
			...
			prepareMock: func(f *fields) {
				...
				f.projectUsecase.EXPECT().UpdateProject(ctx, in).Return(nil, err)
			},
			prepareRequest: func(r *http.Request) {
				*r = *mux.SetURLVars(r, map[string]string{
					"projectID": "024d71d6-1d03-11ec-a478-0242ac1800023",
	...
	tests := []struct {
		...
		},
		{
			...
			prepareMock: func(f *fields) {
				...
				f.projectUsecase.EXPECT().FetchProjects(ctx).Return(nil, err)
			},
		},
	}
	for _, tt := range tests {
